...
#### Description
This tool uses vegetation height, terrain slope, wind speed, and burning condition to map safe separation distance (SSD) across an entire landscape. More detailed information on the calculation of SSD can be found here. SSD will be mapped according to the extent of the input raster datasets (Vegetation Height and Digital Terrain Model (DTM)). It is recommended to use the Download LANDFIRE EVH and DTM from Polygon tool prior to using this tool, so the input raster data are in the correct format. 
#### Parameters
| Name  | Explanation | Data Type
| ------------- | ------------- | ------------- |
...
#### Description
This tool allows users to assess the suitability of a specific target safety zone based on vegetation height, terrain slope, wind speed, and burning condition. Detailed information on the development of SSDE can be found [here](https://www.mdpi.com/2571-6255/5/1/5/pdf). This tool replicates the [Google Earth Engine version](https://firesafetygis.users.earthengine.app/view/ssde-en) of SSDE proposed in Campbell et. al. 2022. Users provide a polygon representing a potential safety zone, raster datasets containing vegetation height and terrain values (such as those downloaded using the Download LANDFIRE EVH and DTM from Polygon tool), a wind speed class, a burning condition class, output file names and locations, and a processing workspace. 
#### Parameters
| Name  | Explanation | Data Type
| ------------- | ------------- | ------------- |
...
The first parameter can either be selected from a folder by clicking on the folder icon and navigating to and selecting the polygon or, if your polygon as already been added to your current map, it can be selected from the dropdown menu. Figure 3 shows example parameters, where the safety zone ‘sz’ has been selected from the current map. Vegetation Height and Digital Terrain Model (DTM) can be selected from the current map or navigated to and selected using the folder icon. If using ‘Download LANDFIRE EVH and DTM from Polygon’ the EVH raster will be in the folder named “landfire_EVH” and the DTM raster in “landfire_DTM” that were created when running the previous tool. Wind speed and burning condition can be selected from the dropdown menu. All three outputs, pSSD, SDD Met, and Safest Point, must be named and placed in appropriate folders or geodatabases. Figure 3 shows all outputs saved in the project folder, including extensions (.tif for raster outputs and .shp for the feature class output). If saving in a geodatabase no extension is needed. Lastly, the workspace must be set to an appropriate folder in which temporary files will be generated. 
